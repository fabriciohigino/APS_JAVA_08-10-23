...
public class ClassePrincipal {
	...
	public static void main(String[] args) {
		...
		System.out.println("Deseja adicionar novos produtos  |"
				+ "\n1-Sim  2-Nao                     |");
		int respAdd = ent.nextInt();
		
		do {
			if (respAdd == 1) {
				System.out.println("1-Alimentos  2-Brinquedos        |");
				respEscolhaProduto = ent.nextInt();
				if (respEscolhaProduto == 1) {
					n1 += 1;
				} else if (respEscolhaProduto == 2) {
					n2 += 1;
				} else {
					continue;
				}
			}
		} while (respEscolhaProduto >= 3);
		...
		while (resp3.equals("s")) {
			...
			case 2:
				...
				System.out.print("Qual o numero do produto? ");
				int resp7 = ent.nextInt();
				System.out.print("Quantas unidades deseja comprar?");
				int resp8 = ent.nextInt();

				if (escolha5 == 1) {
					ali[resp7 - 1].vender(resp8);

				} else if (escolha5 == 2) {
					bri[resp7 - 1].vender(resp8);
				}
				break;
			}
